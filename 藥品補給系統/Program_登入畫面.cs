...
namespace 藥品補給系統
{
    public partial class Form1 : Form
    {
        ...
        void sub_PLC_Program_登入畫面_登入()
        {
            ...
            if (cnt_Program_登入畫面_登入 == 4) cnt_Program_登入畫面_登入_從更新SQL權限資訊開始(ref cnt_Program_登入畫面_登入);
            if (cnt_Program_登入畫面_登入 == 5) cnt_Program_登入畫面_登入_從更新SQL權限資訊結束(ref cnt_Program_登入畫面_登入);
            if (cnt_Program_登入畫面_登入 == 6) cnt_Program_登入畫面_登入 = 100;
        ...
        void cnt_Program_登入畫面_登入_從更新SQL權限資訊開始(ref int cnt)
        {
            if (!PLC_Device_取得SQL登入權限.Bool)
            {
                PLC_Device_取得SQL登入權限.Bool = true;
                cnt++;
            }
        }
        void cnt_Program_登入畫面_登入_從更新SQL權限資訊結束(ref int cnt)
        {
            if (!PLC_Device_取得SQL登入權限.Bool)
            {
                cnt++;
        ...
        void cnt_Program_登入畫面_登入_100_檢查是否為最高權限(ref int cnt)
        {
            ...
            if (ID == this.Admin_ID)
            {
                if(Password == this.Admoin_Password)
                {
                    this.Invoke(new Action(delegate
                    {
                        this.groupBox_伺服器參數.Visible = true;
                    }));
            ...
            this.Invoke(new Action(delegate
            {
                this.groupBox_伺服器參數.Visible = false;
            }));
            cnt++;
        }
